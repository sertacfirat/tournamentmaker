...
const TeamInput: React.FC<TeamInputProps> = ({ value, onChange, label, placeholder }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    onChange(val);
    if (val.length > 0) {
      setSuggestions(POPULAR_TEAMS.filter(t => t.toLowerCase().includes(val.toLowerCase())));
    } else {
      setSuggestions([]);
    }
  };
...
interface Props {
  ...
  matches: Match[];
  players: Player[];
  onUpdateMatch: (id: string, hScore: number, aScore: number, hTeam: string, aTeam: string) => void;
}

const Fixtures: React.FC<Props> = ({ lang, matches, players, onUpdateMatch }) => {
  ...
  const TeamDisplay = ({ team, align }: { team: MatchTeam, align: 'left' | 'right' }) => (
    <div className={`flex flex-col items-center sm:items-${align} text-center sm:text-${align} w-full`}>
       <div className="flex items-center gap-2">
          {align === 'right' && team.isGhost && (
              <div title={t.ghostMatch}>
                  <Ghost size={16} className="text-gray-400" />
              </div>
          )}
          <div className={`font-semibold text-lg leading-tight ${team.isGhost ? 'text-gray-400 dark:text-gray-500 line-through decoration-transparent' : 'text-gray-900 dark:text-gray-100'}`}>
             {team.playerIds.map(id => getPlayerName(id)).join(' & ')}
          </div>
          {align === 'left' && team.isGhost && (
              <div title={t.ghostMatch}>
                  <Ghost size={16} className="text-gray-400" />
              </div>
          )}
    ...
    <div className="space-y-4">
      {filteredMatches.map((match) => (
        <div 
          ...
          className={`group relative bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md transition-all cursor-pointer ${match.isCompleted ? 'opacity-80 hover:opacity-100' : ''}`}
        >
           <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <div className="flex flex-col items-center justify-center min-w-[100px] px-2">
                 {match.isCompleted ? (
                   <div className="text-3xl font-bold font-mono tracking-widest text-brand-600 dark:text-brand-400">
                     {match.home.score} - {match.away.score}
                   </div>
                 ) : (
                   <div className="w-10 h-10 rounded-full bg-gray-100 dark:bg-slate-700 flex items-center justify-center text-xs font-bold text-gray-400">
                     VS
                   </div>
                 )}
                 <div className="mt-1 text-[10px] uppercase font-bold text-gray-300 dark:text-slate-600 tracking-wider">
                   {t.round} {match.round}
                 </div>
              </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
             <div className="p-4 bg-gray-50 dark:bg-slate-700 border-b border-gray-100 dark:border-slate-600 flex justify-between items-center">
                <h3 className="font-bold text-gray-800 dark:text-gray-100">{t.enterScore}</h3>
             ...
             <div className="p-6 space-y-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-center gap-2 font-bold text-center text-brand-700 dark:text-brand-300">
                     ...
                     <span>{editingMatch.home.playerIds.map(id => getPlayerName(id)).join(' & ')}</span>
                  </div>
                  <div className="flex space-x-2">
                     <div className="flex-1">
                        <TeamInput value={hTeam} onChange={setHTeam} label={t.home} placeholder={t.selectTeam} />
                     </div>
                     <div className="w-20">
                        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1 text-center">{t.scoreInput}</label>
                        <input 
                          type="number" 
                          value={hScore} 
                ...
                <div className="space-y-3">
                   <div className="flex items-center justify-center gap-2 font-bold text-center text-red-700 dark:text-red-300">
                     ...
                     <span>{editingMatch.away.playerIds.map(id => getPlayerName(id)).join(' & ')}</span>
                  </div>
                  <div className="flex space-x-2">
                     <div className="flex-1">
                        <TeamInput value={aTeam} onChange={setATeam} label={t.away} placeholder={t.selectTeam} />
                     </div>
                     <div className="w-20">
                        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1 text-center">{t.scoreInput}</label>
                        <input 
